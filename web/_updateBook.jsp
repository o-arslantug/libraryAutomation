    ...
    <SCRIPT LANGUAGE="JavaScript">
        function checkBoxes(form) {
            ...
            if (isNaN(document.F1.bookCurrentNumber.value)) {
                alert("Mevcut sayı değeri yalnızca rakamlardan olaşbilir.")
        ...
        <div class="content_box">
            ...
            <td valign="top">
                    <%
                            ...
                            out.print("<h2>Kitap Güncelle</h2>");
                    ...
                    <table height="0" cellspacing="5" cellpadding="10">
                        ...
                        <%
                            ...
                            try {
                                ...
                                PreparedStatement preparedStatement = connection.prepareStatement("SELECT kitap_ad, kitap_isbn, kitap_basimYili, kitap_id FROM Kitap WHERE kitap_id = ?");
                                String bookID = request.getParameter("bookID");
                                preparedStatement.setString(1, bookID);
                                PreparedStatement preparedStatementControl = connection.prepareStatement("SELECT kitap_ad, kitap_isbn, kitap_basimYili, kitap_id FROM Kitap WHERE kitap_id = ?");
                                preparedStatementControl.setString(1, bookID);
                        ...
                        <tr>
                            <td>Kitap Adı:</td>
                            <td><input type="text" name="bookName" value="<%=resultSet.getString(1)%>" required placeholder="Bu alanı boş bırakılamaz."/></td>
                        </tr>
                        <tr>
                            <td>ISBN No:</td>
                            <td><input type="text" name="bookISBN" value="<%=resultSet.getString(2)%>" required placeholder="Bu alanı boş bırakılamaz."/></td>
                        </tr>
                        <tr>
                            <td>Basım Yılı:</td>
                            <td><input type="text" name="bookPublishingYear" value="<%=resultSet.getString(3)%>" required placeholder="Bu alanı boş bırakılamaz."/></td>
                        </tr>
                        <input type="hidden" name="bookID" value="<%=resultSet.getString(4)%>" readonly="readonly"/>
                        <%
                                }
                        ...
                        <tr>
                            ...
                            <td><select name="bookTranslatorID" class="form-control" style=width:177px;">
                                ...
                                <% try {
                                    ...
                                    PreparedStatement ps = con.prepareStatement("SELECT * FROM Cevirmen");
                                    ResultSet rs = ps.executeQuery();

                                    while (rs.next()) {
                                %>
                                <option value="<%=rs.getString("cevirmen_id")%>"><%=rs.getString("cevirmen_ad")%>
                                </option>
                                <%
                                        }
                                    } catch (Exception e) {
                        ...
                        <tr>
                            ...
                            <td><select name="bookPublisherID" class="form-control" style=width:177px;">
                                ...
                                <%
                                    try {
                                        Connection con = AccessLayer.getConnection();
                                        PreparedStatement ps = con.prepareStatement("SELECT * FROM YayinEvi");
                                        ResultSet rs = ps.executeQuery();

                                        while (rs.next()) {
                                %>
                                <option value="<%=rs.getString("yayinEvi_id")%>"><%=rs.getString("yayinEvi_ad")%>
                                </option>
                                <%
                                        }
                                    } catch (Exception e) {
                            ...
                            <td><select name="bookAuthorID" class="form-control" style=width:177px;">
                                <option value="-1">Yazar Seçiniz</option>
                                    <%
                                                try {
                                                    Connection con = AccessLayer.getConnection();
                                                    PreparedStatement ps = con.prepareStatement("SELECT * FROM Yazar");
                                                    ResultSet rs = ps.executeQuery();

                                                    while (rs.next()) {
                                            %>
                                <option value="<%=rs.getString("yazar_id")%>"><%=rs.getString("yazar_ad")%>
                                </option>
                                    <%
                                                    }
                                                } catch (Exception e) {
                                                ...
                                                try {
                                                    Connection connection = AccessLayer.getConnection();
                                                    PreparedStatement preparedStatement = connection.prepareStatement("SELECT kitap_kategori, kitap_konum, kitap_mevcutsayi FROM Kitap WHERE kitap_id = ?");
                                                    String bookID = request.getParameter("bookID");
                                                    preparedStatement.setString(1, bookID);
                                                    ResultSet resultSet = preparedStatement.executeQuery();
                        ...
                        <tr>
                            <td>Kategori:</td>
                            <td><input type="text" name="bookCategory" value="<%=resultSet.getString(1)%>" required placeholder="Bu alanı boş bırakılamaz."/></td>
                        </tr>
                        <tr>
                            <td>Konum:</td>
                            <td><input type="text" name="bookLocation" value="<%=resultSet.getString(2)%>" required placeholder="Bu alanı boş bırakılamaz."/></td>
                        </tr>
                        <tr>
                            <td>Mevcut Sayı:</td>
                            <td><input type="text" name="bookCurrentNumber" value="<%=resultSet.getString(3)%>" required placeholder="Bu alanı boş bırakılamaz."/></td>
                        </tr>
                        <input type="hidden" name="comingFrom" value="updateBook" readonly="readonly"/>
                        <%
                                }
                            } catch (SQLException e) {
                                e.printStackTrace();
                        ...
                        <tr>
                            ...
                            <td><input type="submit" value="Gönder"/>
                                <INPUT type="reset" value="Geri Al"></td>
                        </tr>
                    </table>
                </form>
